...
##' @param rec_from_par if TRUE the recruitment is drawn using parameter input
##' @param switch_year_range if TRUE, the range interval for catch mean weight is one less then stock mean weight
##' @param bias_correct if TRUE, the initial parameters are subtracted with 0.5*var
...
forecast_XSAM <- function(fit,fscale=NULL, catchval=NULL, catchval.exact=NULL, fval=NULL,fBarW = NULL, nextssb=NULL, landval=NULL, cwF=NULL, nosim=1000, year.base=max(fit$data$years), ave.years=max(fit$data$years)+(-4:0), rec.years=max(fit$data$years)+(-9:0), label=NULL, overwriteSelYears=NULL, deterministic=FALSE, processNoiseF=TRUE, customWeights=NULL, customSel=NULL, lagR=FALSE, splitLD=FALSE, addTSB=FALSE,rec_from_par=F,switch_year_range=F,bias_correct=T){
  ...
  # }
  #Added by Sindre. We have replaced the fbar function to compute the fbar weighted. 
  #In future, the fbarW function should be made instead and implemented in the 
  #subsequent code
  fbar<-function(x){
    N <- getN(x)
  ...
  step <- function(x, nm, recpool, scale, inyear=FALSE){
    ...
    if(!inyear){
      Z <- F+nm
      n <- length(N)
      # N <- c(median(recpool),N[-n]*exp(-Z[-n])+c(rep(0,n-2),N[n]*exp(-Z[n])))
  ...
  recpool<-exp(rnorm(10000,as.numeric(recpars[2]) ,exp(as.numeric(recpars[1]))))
  # recpool<-exp(rnorm(10000,as.numeric(recpars[2]) -0.5*exp(as.numeric(recpars[1]))^2,exp(as.numeric(recpars[1]))))
  if(deterministic)recpool = exp(as.numeric(recpars[2]))
  }
  # Get final state
  ...
  #Added by sindre
  if(bias_correct)est <- est-0.5*(diag(cov))
  ...
  #Added by sindre
  if(switch_year_range)ave.years<-ave.years-1
  ...
  for(i in 0:(length(fscale)-1)){
    ...
    fatagesim <- apply(sim, 1, getF) #Added by sindre
    ...
    simlist[[i+1]] <- list(sim=sim, fbar=fbarsim, catch=catchsim, ssb=ssbsim, rec=recsim,
                           cwF=cwFsim, catchatage=catchatagesim,fatage = fatagesim, #fatage is added by sindre
                           land=landsim, fbarL=fbarLsim, tsb=tsbsim, year=y)
  ...
  #added by sindre
  faytable<-round(do.call(rbind, lapply(simlist, function(xx)apply(xx$fatage,1,collect))),digits = 3) 
  ...
  attr(simlist, "faytable")<-faytable #added by sindre
...
forecastBio<-function (fit, fscale = NULL, catchval = NULL, catchval.exact = NULL, 
                       ...
                       cf.keep.fv.offset = matrix(0, ncol = sum(fit$data$fleetTypes == 
                                                                  0), nrow = length(catchval)), estimate = median) 
{
  ...
  doAve <- function(x) {
    if (length(dim(x)) == 2) {
      ret <- colMeans(x[rownames(x) %in% ave.years, , 
  ...
  for (i in 0:(length(fscale) - 1)) {
    ...
    if (!is.na(catchval.exact[i + 1])) {
      ...
      funk <- function(k) {
        ...
        one <- function(s) {
          simtmp[k, ] <<- scaleF(sim[k, ], s)
          simcat <- catch(simtmp[k, ], nm = thisnm, 
                          cw = thiscw)
          return(catchval.exact[i + 1] - simcat)
        }
        ff <- uniroot(one, c(0, 100))$root
    ...
    simlist[[i + 1]] <- list(sim = sim, fbar = fbarsim, 
                             fbarbyfleet = fbarbyfleetsim, catch = catchsim, 
                             ssb = ssbsim, rec = recsim, cwF = cwFsim, catchatage = catchatagesim, 
                             land = landsim, fbarL = fbarLsim, tsb = tsbsim, 
                             catchby = catchbysim, year = y,swtab = swsim)
  }
  attr(simlist, "fit") <- fit
  collect <- function(x) {
  ...
  caytable <- round(do.call(rbind, lapply(simlist, function(xx) apply(xx$catchatage, 
                                                                      1, collect))))
  # swtab <- round(do.call(rbind, lapply(simlist, function(xx) collect(xx$sw))))
  # print(swtab)
  tab <- cbind(fbar, rec, ssb, catch)
  if (splitLD) {
